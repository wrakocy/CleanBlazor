...
`Core/<Domain>/...`, `Infrastructure/...`, `Web/...`. See
...
7. **Integration tests** (real dependencies) go in `IntegrationTests` instead, using the same
   `<Class>_<Method>.cs` naming — see `add-integration-test`. **Functional/full-stack tests**
   (`WebApplicationFactory`) go in `FunctionalTests` — see `add-functional-test`.
