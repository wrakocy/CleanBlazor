---
name: add-persistence-integration
...
Which path applies depends on this solution's persistence model (check `AGENTS.md`'s "Persistence
model" section). A single solution may use both, decided per domain.
...
5. **Test it** — see `add-integration-test` for a real-database round-trip test, and
   `add-unit-test` for handler tests that mock `I<Entity>Repository`.
...
4. **Initialize** it at startup: add a call in the app's cache-initialization step (invoked once
   ...
   serving traffic.
5. Have `add-domain-event` handlers call its `Notify...Changed` methods after a write, and have
   query handlers for this domain read through the cache instead of hitting the API per request.
