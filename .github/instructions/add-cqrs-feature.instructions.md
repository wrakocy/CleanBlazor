...
3. **Write the handler** in `<UseCaseName>Handler.cs`, implementing
   ...
   publish a domain event afterward via `_appBus.Publish(...)` (see
   `add-domain-event.instructions.md`) — do not call `IMediator` directly.
...
5. **Call it from Blazor** via `_appBus.Send(new CreateOrderCommand(model))` from a component's
   code-behind (see `add-blazor-page.instructions.md`), never by injecting `IMediator`.
6. **Add tests** — see `add-unit-test.instructions.md`. At minimum: a constructor test for the
   request (null-arg guard behavior) and a `Handle` test for the handler (happy path + null-arg
   guard).

## Gotchas

- Validators are separate from the command/query — see
  `add-fluentvalidation-validator.instructions.md`; a command constructor only null-guards, it
...
- Keep commands and queries in this one instructions file rather than splitting into separate
  "add-command"/"add-query" files — they share the same folder-per-use-case shape and mostly
